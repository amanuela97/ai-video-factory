...
// Renders the ByteForge outro card with a voiceover and optional logo overlay.
// audioPath: ElevenLabs-generated outro audio (.mp3)
// logoPath: channel logo PNG — pass null to render text-only fallback.
// Duration is driven by the audio length via -shortest.
export async function renderOutro(audioPath: string, logoPath: string | null): Promise<string> {
  const scenesDir = path.resolve("./tmp/scenes");
  fs.mkdirSync(scenesDir, { recursive: true });
  ...
  if (logoPath) {
    // Three inputs: [0]=bg colour, [1]=audio, [2]=logo
    const filterComplex = [
    ...
    // Text-only fallback (no logo input)
    const vf = [
      "drawtext=text='ByteForge':fontcolor=white:fontsize=56:x=(w-text_w)/2:y=(h/2)-30",
      "drawtext=text='Subscribe for more':fontcolor=0xaaaaaa:fontsize=34:x=(w-text_w)/2:y=(h/2)+40",
    ].join(",");
  ...
  console.log(`Rendering ByteForge outro${logoPath ? " with logo" : " (text-only fallback)"}...`);
  execSync(cmd, { stdio: "inherit" });
  return output;
}
